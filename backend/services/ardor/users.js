...
async function getActiveUsers(period = 'all', forceRefresh = false) {
  try {
    // Fix: Pass the actual function reference, not the result
    return await getCachedData(
      'users',
      fetchAllUsersData, // Pass function reference here
      {
        forceRefresh,
        period
      }
...
async function fetchAllUsersData(period = 'all') {
  try {
    console.log(`Fetching all users data for period ${period}...`);
...
module.exports = { getActiveUsers, getAssetTrades, getAssetTransfers };